...
class ExpenseCategory(models.Model):
    ...
    lastAddedTo = models.DateTimeField(default=datetime.now)
    class Meta:
...
class MemberOfGroup(models.Model): # if the name given by one person already exists for another member of the group the name should be able to get a different identifier for the user 
    ...
    paid_off = models.BooleanField(default=False) # the user has paid off their part chnage to true
    lastPayment = models.DateTimeField(default=datetime.now)
    money_spent = models.DecimalField(max_digits=10,decimal_places=2, null=True, blank=True, default=0)
...
class Group(models.Model):
    ...
    money_per_user = models.DecimalField(max_digits=10,decimal_places=2, null=True, blank=True)
